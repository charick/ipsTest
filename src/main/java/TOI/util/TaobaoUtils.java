...
public class TaobaoUtils
{
    ...
    private TaobaoUtils(String topSession){
    ...
    public  Map<String, String> getCCMapFromFile()
    {
        Map result = new HashMap();
        try
        {
            InputStreamReader insReader = new InputStreamReader(new FileInputStream(new File(Constant.resourcePath+"category/tb_category")), "utf-8");
    ...
    public  void saveTBcategory2File() {
        try {
            ArrayList<SellerCid> tbclist = getSellerCid();
            FileWriter fw = new FileWriter(new File(Constant.resourcePath+"category/tb_category"));
    ...
    public String addTaobaoItem(Product p)
    ...
    {         String num_iid="0";
        ...
        FileItem fItem = new FileItem(new File(Constant.picPath+"ITEMPICS/" + ((TOI.model.Item)p.itemsList.get(0))
                .pid + "-0.jpg"));
        System.out.println(Constant.picPath+"ITEMSPIC/" + ((TOI.model.Item)p.itemsList.get(0)).pid + "-0.jpg");
        req.setImage(fItem);

        req.setOuterId(p.pid);
        try
        {
            ItemAddResponse response = (ItemAddResponse)client.execute(req, topSession);
            ...
            try
            {
               ...
               num_iid = item.getString("num_iid");
    ...
    public  void getOnlineProducts()
    {
        ...
        try
        {
            ItemsOnsaleGetResponse response = (ItemsOnsaleGetResponse)client.execute(req,topSession);
     ...
     * 更新线上产品
     */
    public void updateOnlineItem(Product product)
    {
        TaobaoClient client = new DefaultTaobaoClient(Constant.url, Constant.appkey, Constant.appSecret);
        if (product.tid != null) {
            ...
            try
            {
                ItemUpdateResponse response = (ItemUpdateResponse)client.execute(req,topSession);
                System.out.println(response.getBody());
            }
            catch (ApiException e)
            {
    ...
    private  ArrayList<SellerCid> getSellerCid()
    {
    ...
    public  void deleteUnusePics()
    {
        ...
        try
        {
            PictureGetResponse response = (PictureGetResponse)client.execute(req,topSession);
            System.out.println(response.getBody());
            ...
            for (int i = 0; i < pictureList.size(); i++) {
                ...
                PictureIsreferencedGetResponse response1 = (PictureIsreferencedGetResponse)client.execute(req1,topSession);
                ...
                if (!response1.getIsReferenced().booleanValue()) {
                    ...
                    PictureDeleteResponse response2 = (PictureDeleteResponse)client.execute(req2,topSession);
    ...
    public  void uploadExtraPic(Product p) {
        TaobaoClient client = new DefaultTaobaoClient(Constant.url, Constant.appkey, Constant.appSecret);
        ...
        for (int i = 1; (i < p.itemsList.size()) && (i < 5); i++) {
            ...
            FileItem fItem = new FileItem(new File(Constant.picPath+"ITEMPICS/" + ((TOI.model.Item)p
                    .itemsList
                    .get(i))
                    .pid + "-0.jpg"));

            System.out.println(Constant.picPath+"ITEMSPIC\\" + ((TOI.model.Item)p.itemsList.get(i)).pid + "-0.jpg");
            ...
            try {
                ItemImgUploadResponse response = (ItemImgUploadResponse)client.execute(req,topSession);
                System.out.println(response.getBody());
            }
            catch (ApiException e)
            {
    ...
    public List<Trade> TradeFilter(String start, String end, String type,String field)
    {
        try
        {
            TaobaoClient client = new DefaultTaobaoClient(Constant.url, Constant.appkey, Constant.appSecret);
